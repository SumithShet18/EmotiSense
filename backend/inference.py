...
def predict_emotion(
...
) -> tuple[str, float, dict[str, float], Optional[str], list[dict], dict, dict]:
    stages: list[dict] = []
    ...
    modality_contribution = {
        "text_pct": round(float(contributions["text_pct"][0]), 1),
        "audio_pct": round(float(contributions["audio_pct"][0]), 1),
        "gate_mean": round(float(contributions["gate_mean"][0]), 3),
    }
    ...
    return emotion_label, confidence, prob_dict, transcript, stages, totals, modality_contribution
